...
export type PendingGamesState = Readonly<Game[]>

export const findPendingGame = (id: string, state: PendingGamesState): Game | undefined => _.find(_.matches({id}), state)

const init_state: PendingGamesState = []

const pending_games_reducers = {
  reset(_state: PendingGamesState, action: PayloadAction<Game[]>): PendingGamesState{
    return action.payload
  },

  upsert(state: PendingGamesState, action: PayloadAction<Game>): PendingGamesState {
